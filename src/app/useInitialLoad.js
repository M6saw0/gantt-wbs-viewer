...
export function useInitialLoad() {
    ...
    const { setProjects, setTasks, setHolidays, setGraph, setCpmResult, setDisplayData, setErrors, setLoading, setLoadError, clearData } = useDataStore();
    const { showToast } = useUiStore();
    const workbookRef = useRef(null);
    const loadFromWorkbook = useCallback(async (workbook, forceProjectId) => {
        try {
            setLoading(true);
            setLoadError(null);
            clearData();
            ...
            try {
                ...
                const displayData = deriveDisplayData(cpmResult.tasks, graph.edges, settings.zoom, 1200, (currentConfigLocal?.rangeBeforeDays) || 7, (currentConfigLocal?.rangeAfterDays) || 45);
    ...
    const loadData = useCallback(async (forceProjectId) => {
    ...
    useEffect(() => {
        ...
        const currentSettings = useSettingsStore.getState().settings;
        if (cpmResult && graph) {
            const displayData = deriveDisplayData(cpmResult.tasks, graph.edges, currentSettings.zoom, 1200, config?.rangeBeforeDays || 7, config?.rangeAfterDays || 45);
            setDisplayData(displayData);
        }
